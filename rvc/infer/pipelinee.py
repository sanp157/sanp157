...
class AudioProcessor:
    ...
    def change_rms(source_audio, source_rate, target_audio, target_rate, rate):
        ...
        frame_length1 = min(len(source_audio), max(2, int(max(2, source_rate // 100))))
        hop_length1 = max(1, frame_length1 // 2)
        frame_length2 = min(len(target_audio), max(2, int(max(2, target_rate // 100))))
        hop_length2 = max(1, frame_length2 // 2)

        rms1 = librosa.feature.rms(
            y=source_audio, frame_length=frame_length1, hop_length=hop_length1
        )
        rms2 = librosa.feature.rms(
            y=target_audio, frame_length=frame_length2, hop_length=hop_length2
        )

        rms1_t = torch.from_numpy(rms1).float().unsqueeze(0).unsqueeze(0)
        rms2_t = torch.from_numpy(rms2).float().unsqueeze(0).unsqueeze(0)

        target_len = int(target_audio.shape[0])
        rms1_i = F.interpolate(
            rms1_t, size=target_len, mode="linear", align_corners=False
        ).squeeze()
        rms2_i = F.interpolate(
            rms2_t, size=target_len, mode="linear", align_corners=False
        ).squeeze()

        eps = 1e-6
        rms1_i = torch.maximum(rms1_i, torch.zeros_like(rms1_i) + eps)
...
class Pipeline:
    ...
    def pipeline(
        ...
        # AUTHOR-SAFE FIX: HuBERT gets UNPADDED audio
